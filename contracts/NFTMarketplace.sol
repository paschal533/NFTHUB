...
contract NFTMarketplace is ERC721URIStorage, ReentrancyGuard {
    ...
    uint256 public totalStakedToken = 0 ether;
    address[] public players;
    uint256 public startTime;
    uint256 public duration = 1 weeks;

    IERC20 public immutable rewardsToken = ERC20(0x19C653Da7c37c66208fbfbE8908A5051B57b4C70);
    ...
    function joinGame() public payable {
      require(msg.value == listingPrice, "not enough fund sent to join game");
      players.push(msg.sender);
      totalStakedToken += msg.value;
    }
    ...
    //runs every week to send token to the game winner
    function endOfGame(address payable _winner) public {
      require(msg.sender == owner, "Only owner can call this function");
      require(block.timestamp >= startTime + duration, "Time period has not elapsed yet");
      require(_winner != address(0), "Invalid recipient address");
      require(address(this).balance >= totalStakedToken, "Insufficient balance in the contract");

      // Transfer Ether to the recipient
      _winner.transfer(totalStakedToken);
