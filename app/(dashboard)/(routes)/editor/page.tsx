...
export default function Home() {
  ...
  const { setBaseImageUrl, setDownloadLink, downloadLink, baseImageUrl } =
  ...
  const calcCustomRes = (res: any) =>
    res < 720 ? "SD" : res < 1920 ? "HD" : res < 3840 ? "FHD" : "UHD";

  const getFileName = (file: any) => file.substr(0, file.lastIndexOf(".")) || x;
  ...
  const heroTitle = `${styles.heroTitle} text-black text-bold`;
  const heroSubTitle = `${styles.heroSubTitle} text-black text-bold`;
  const descriptionBox = `${styles.descriptionBox} text-black text-bold`;

  const pushToMint = async (link: string, image: string) => {
    setDownloadLink(link);
    setBaseImageUrl(image);

    if (baseImageUrl.length > 1 && downloadLink.length > 1) {
            ...
            <div className={styles.heroSelect}>
              <div className={styles.heroDescriptionArea}>
                ...
                <DescriptionCard
                  ...
                  description="Let NFTHUB sort it all out for you!"
                />
              </div>
          ...
          <div className={styles.toolsArea}>
            ...
            <div className={styles.downloadArea}>
              <Button
                ...
                w="100%"
                onClick={() =>
                  pushToMint(`${fileName}-cropped.${fileType}`, baseImage)
                }
              >
                Mint 🎉
              </Button>
